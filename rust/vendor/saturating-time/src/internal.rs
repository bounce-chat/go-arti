    ...
    F: Fn(&T, Duration) -> Option<T>,
{
    ...
    loop {
        ...
        // A step smaller than the platform's clock granularity succeeds while
        // moving nothing: Windows stores `SystemTime` as a FILETIME of 100ns
        // ticks, so subtracting 1ns truncates to zero and hands back the value
        // unchanged. Treating that as progress means the step never halves and
        // this loop never ends, so a stalled step is treated exactly like the
        // out-of-range case below.
        let stalled = matches!(&next, Some(st) if *st == res);
...
#[cfg(test)]
mod coarse_granularity_tests {
    use super::*;

    /// A clock whose smallest representable unit is 100 ticks, mimicking the
    /// Windows FILETIME granularity that made `find_limit` loop forever: an
    /// operation smaller than one tick succeeds but changes nothing.
    #[derive(Clone, Copy, PartialEq, Debug)]
    struct CoarseTime(i64);
    ...
    impl CoarseTime {
        ...
        fn ticks(d: Duration) -> i64 {
            (d.as_nanos() / Self::TICK_NANOS).min(i64::MAX as u128) as i64
        }
    }
    ...
    impl SaturatingTime for CoarseTime {
        ...
        fn max_value() -> Self {
            CoarseTime(Self::LIMIT)
        }
        fn min_value() -> Self {
            CoarseTime(-Self::LIMIT)
        }
        fn checked_add(&self, duration: Duration) -> Option<Self> {
            let v = self.0.checked_add(Self::ticks(duration))?;
            (v <= Self::LIMIT).then_some(CoarseTime(v))
        }
        fn checked_sub(&self, duration: Duration) -> Option<Self> {
            let v = self.0.checked_sub(Self::ticks(duration))?;
            (v >= -Self::LIMIT).then_some(CoarseTime(v))
        }
        fn checked_duration_since(&self, earlier: Self) -> Option<Duration> {
            let delta = self.0.checked_sub(earlier.0)?;
            (delta >= 0).then(|| Duration::from_nanos((delta as u128 * Self::TICK_NANOS) as u64))
        }
    }

    /// Before the no-progress check these looped forever, because a 1ns step
    /// truncates to zero ticks and so always "succeeds".
    #[test]
    fn terminates_when_the_step_is_below_clock_granularity() {
        assert_eq!(find_min::<CoarseTime>(), CoarseTime(-CoarseTime::LIMIT));
        assert_eq!(find_max::<CoarseTime>(), CoarseTime(CoarseTime::LIMIT));
    }

    /// A step at or above one tick must still be treated as progress.
    #[test]
    fn a_step_of_one_tick_still_makes_progress() {
        let a = CoarseTime(0);
        assert_eq!(a.checked_sub(Duration::from_nanos(100)), Some(CoarseTime(-1)));
        assert_eq!(a.checked_sub(Duration::from_nanos(1)), Some(CoarseTime(0)));
    }
}
